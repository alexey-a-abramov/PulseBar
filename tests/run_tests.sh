...
echo "------------------------------------------------------------"
clang -fobjc-arc -O0 -isysroot "$SDK" \
  "$TESTS/layout_test.m" "$SRC/BarView.m" "$SRC/PBDefaults.m" "$SRC/PBProcess.m" "$SRC/PBFormat.m" "$SRC/PBLayout.m" "$SRC/PBClock.m" "$SRC/PBThermal.m" "$SRC/AppIndex.m" "$SRC/Log.m" "$SRC/Stats.m" "$SRC/Controls.m" "$SRC/Pomodoro.m" \

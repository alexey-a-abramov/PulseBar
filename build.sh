...
clang -fobjc-arc -O2 -mmacosx-version-min=12.0 -isysroot "$SDK" \
  ...
  "$SRC/PBDefaults.m" "$SRC/PBProcess.m" "$SRC/PBFormat.m" "$SRC/PBLayout.m" "$SRC/PBClock.m" "$SRC/PBThermal.m" "$SRC/Log.m" "$SRC/Stats.m" "$SRC/Controls.m" "$SRC/Pomodoro.m" \
  ...
  "$SRC/PBSpeechCapture.m" "$SRC/PBAgentHUD.m" "$SRC/PBOllama.m" \
